...
async function updateSubscriptionWithRetry(oldSubscription: PushSubscription | null, newSubscription: PushSubscription, retryInterval: number): Promise<void> {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Server responded with status ${response.status} during subscription update`);
    }

    console.log('Subscription updated successfully');
  } catch (error) {
    console.error('Failed to update subscription: ', error);
    console.log(`Retrying in ${retryInterval / 60000} minutes`);

    await new Promise(resolve => setTimeout(resolve, retryInterval));
    await updateSubscriptionWithRetry(oldSubscription, newSubscription, retryInterval);
  }
}
...
self.addEventListener('pushsubscriptionchange', async (event) => {
  ...
  event.waitUntil(self.registration.pushManager.subscribe(event.oldSubscription?.options)
      .then(newSubscription => {
        ...
        //@ts-ignore
        return updateSubscriptionWithRetry(event.oldSubscription, newSubscription, minutes * 60 * 1000)
      })
  )
});
